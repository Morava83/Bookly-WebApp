...
def create_group_meeting():
    ...
    # Normalize slot data.
    # New format uses slot["date"].
    # Old format uses slot["start_date"].
    normalized_slots = []
    ...
    if is_recurring:
        ...
        # For recurring meetings, GroupMeeting.endDate should mean the repeat-until date
    ...
    try:
        ...
        # This is not finalized yet, so status = open.
        ...
        # 2. Create GroupMeeting record with recurrence settings.
        # numOfRecurrences is calculated later, when owner picks the final slot.
        cursor.execute("""
        ...
        # 3. Insert voting availability options.
        ...
        for email in invitees:
            ...
            if not student_id:
                # Ignore invalid/non-student emails
                continue
...
# GET owner-created group meetings
...
def owner_group_meetings():
    ...
    try:
        cursor.execute("""
            ...
            JOIN Meeting m
                ON m.meetingID = gm.meetingID
            LEFT JOIN GroupInvite gi
                ON gi.meetingID = gm.meetingID
...
def decide_meeting():
    ...
    try:
        # 1. Check owner owns this group meeting and get recurrence settings.
        cursor.execute("""
            SELECT
                ownerID,
                title,
        ...
        # 3. Calculate recurrence count if needed.
        ...
        if int(meeting_row["isRecurring"] or 0) == 1:
            ...
            recurrence_text = f"weekly × {num_recurrences}"
        ...
        # 5. Mark selected availability as booked.
        cursor.execute("""
            UPDATE Availability
            SET status = 'booked'
            WHERE availabilityID = ?
        ...
        # 8. Save calculated recurrence count.
        cursor.execute("""
            UPDATE GroupMeeting
            SET numOfRecurrences = ?
            WHERE meetingID = ?
        """, (
            num_recurrences,
            meeting_id
        ...
        # 9. Create Booking2 rows for students who voted for the selected slot.
        cursor.execute("""
        ...
        # 10. Send notification email to owner.
        cursor.execute("""
            SELECT email
            FROM User
            WHERE userID = ?
...
def submit_vote():
    ...
    try:
        ...
        # Clear previous votes for this meeting, so resubmitting replaces choices
        cursor.execute("""
            DELETE FROM Vote
            WHERE studentID = ?
        ...
        for availability_id in availability_ids:
            ...
            cursor.execute("""
                INSERT INTO Vote (studentID, availabilityID)
                VALUES (?, ?)
    ...
    try:
        cursor.execute("""
            ...
            JOIN GroupMeeting gm
                ON gm.meetingID = b.meetingID
            JOIN Meeting m
                ON m.meetingID = b.meetingID
            JOIN User u
                ON u.userID = b.ownerID
            WHERE b.studentID = ?
...
def my_group_invites():
    ...
    try:
        cursor.execute("""
            ...
            FROM GroupInvite gi
            JOIN GroupMeeting gm
                ON gm.meetingID = gi.meetingID
            JOIN Meeting m
                ON m.meetingID = gm.meetingID
            JOIN User u
                ON u.userID = gm.ownerID
            LEFT JOIN Availability a
                ON a.meetingID = gm.meetingID
            LEFT JOIN Vote v
                ON v.availabilityID = a.availabilityID
               AND v.studentID = gi.studentID
            WHERE gi.studentID = ?
              AND m.status = 'open'
            GROUP BY
                gm.meetingID,
...
def owner_group_bookings():
    ...
    try:
        cursor.execute("""
            ...
            JOIN Meeting m
                ON m.meetingID = gm.meetingID
            LEFT JOIN Booking2 b
                ON b.meetingID = gm.meetingID
            LEFT JOIN User u
                ON u.userID = b.studentID
            WHERE gm.ownerID = ?
              AND m.status IN ('booked', 'cancelled')
            GROUP BY
                gm.meetingID,
                gm.title,
                gm.description,
